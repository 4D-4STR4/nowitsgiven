# ONDS — Ondas Inc. Deep-Dive Dossier

**Ticker:** NASDAQ: ONDS · **Formerly:** Ondas Holdings Inc. (renamed Jan 2026) · **As of:** 2026-06-19

A multi-source, cross-verified research dossier built from SEC filings (10-K/10-Q/8-K),
company IR, and reputable trade/financial press. Every material claim is source-cited;
unverifiable items are explicitly flagged `UNVERIFIED`.

## Sections
| # | Section | File |
|---|---------|------|
| 1 | Financials & SEC Filings | [`sections/01-financials-sec.md`](./sections/01-financials-sec.md) |
| 2 | Business Model, Segments & Technology | [`sections/02-business-and-technology.md`](./sections/02-business-and-technology.md) |
| 3 | TAM, SAM & Market Sizing | [`sections/03-tam-market-sizing.md`](./sections/03-tam-market-sizing.md) |
| 4 | Competitive Landscape | [`sections/04-competition.md`](./sections/04-competition.md) |
| 5 | Execution History, Management & Capital Structure | [`sections/05-execution-management.md`](./sections/05-execution-management.md) |
| 6 | Catalysts, Contracts, M&A & Events | [`sections/06-catalysts-ma-contracts.md`](./sections/06-catalysts-ma-contracts.md) |
| 7 | Industry, Regulatory & Macro Lateral Factors | [`sections/07-industry-regulatory-macro.md`](./sections/07-industry-regulatory-macro.md) |
| ⭐ | **Executive Summary / Master Dossier** | [`DOSSIER.md`](./DOSSIER.md) |
| 📌 | **One-Page Study Cheat-Sheet** | [`STUDY-GUIDE.md`](./STUDY-GUIDE.md) |

> **Start here:** skim [`STUDY-GUIDE.md`](./STUDY-GUIDE.md) (~5 min) → read [`DOSSIER.md`](./DOSSIER.md) for the executive synthesis → then drill into sections.

### 📱 PDF versions (phone-friendly)
Pre-rendered PDFs are in [`pdf/`](./pdf/) — including a single combined **`ONDS-Full-Dossier.pdf`** (everything in one file) plus individual PDFs per document. Regenerate them with:
```
pip install -r ../scripts/requirements.txt
python3 ../scripts/md_to_pdf.py
```

## Snapshot (verified, as of 2026-06-19)
- **Q1 2026 revenue:** $50.1M (+~1,065% YoY); FY2025 $50.7M (+605%); gross margin ~49% Q1-26.
- **Profitability:** Operating loss $(42.7)M in Q1-26; the +$362.8M "net income" is a **non-cash** warrant/deconsolidation artifact.
- **Balance sheet:** ~$1.48B cash + ST investments; minimal debt; **~$1.06B warrant liability**.
- **Dilution:** ~93M (YE2024) → ~496M shares (May 2026); >$1.8B raised since mid-2025.
- **Backlog:** ~$457M pro forma (Q1-26), heavily M&A-driven (Mistral).
- **Structure:** Ondas Networks **deconsolidated** Jan 16, 2026 (Ondas retains ~47.5%); OAS defense/autonomy is the core.
- **Flags:** J Capital short report (Feb 2026); Cramer "meme stock"; ~33% short interest; serial historical guidance misses.

## Method & caveats
Compiled by parallel specialist research agents, each restricted to source-cited claims with
explicit `UNVERIFIED` flags. Press-release figures (backlog, guidance, Adjusted EBITDA) are
labeled as company-provided/non-GAAP, distinct from audited SEC figures. **Not financial advice.**
